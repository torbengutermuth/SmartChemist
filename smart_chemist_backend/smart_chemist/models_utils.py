...
def make_pattern_matching_job(request_data, nof_molecules_allowed: int = 100) -> Type[PatternMatchingJob]:
    ...
    if "smiles" in request_data:
        ...
        S1C2NCCCC2CC1 Poster Example,
        c1cc(C)ccc1c2cc(C(F)(F)F)nn2c3ccc(cc3)S(=O)(=O)N Toc Example,
        ...
        S(=O)(=O)(N)c1c(N)cc(c(S(=O)(=O)N)c1)C(F)(F)F 2POW_I7C_A_1000"""
        job.input_info.input_string = ",".join(random.sample(all_molecules_string.split(","), k=3))
    job.input_info.save()
    job.save()
    return job
